...
T& Array<T>::operator[](int i)
{
  ...
  if(i >= size || i < 0)
    {
      std::cout<<"Array::[] - [i] invalido"<<std::endl;
      exit(9);
    }
#endif
  return pType[i];
...
const T& Array<T>::operator[](int i) const
{
  ...
  if(i >= size || i< 0)
    {
      std::cout<<"Array::[] const - [i] invalido"<<std::endl;
      exit(5);
    }
#endif
  return pType[i];
...
void Array<T>::realoca(const int novo_tam, const T val)
{
  ...
  if(novo_tam <= size)
    {
      std::cout<<"Array::realoca - Tamalho menor do que o existente"<<std::endl;
      exit(9);
    }
  T* novo_pType = new(std::nothrow) T[novo_tam];
  if(novo_pType == NULL)
    {
      std::cout<<"Array::realoca - Falta de memoria"<<std::endl;
      exit(0);
    }
#ifdef DEBUG_ARRAY
  //++naloc;
  ...
  if(novo_tam != 0)
    {
      ...
      if(pType == NULL)
	{
	  std::cout<<"Array::aloca - Falta de memoria"<<std::endl;
	  exit(0);
	}
#ifdef DEBUG_ARRAY
      //      ++naloc;
...
void Array<T>::set(const T val, const int fim, const int ini)
{
  ...
  if(ini < 0 || fim > size)
    {
      std::cout<<"Array::set - liminte invalido"<<std::endl;
      exit(9);
    }
#endif
...
void Array<T>::ajusta(const int novo_tam)
{
  ...
  if(novo_tam >= size)
    mserro("Array","ajusta","Tamalho menor do que o existente",9);
  if(novo_tam != 0)
    {
